...
void KeyFrameGraphDisplay::draw()
{
	...
	if(flushPointcloud)
	{
		...
		printf("Flushing Pointcloud to %s!\n", (ros::package::getPath("ar_viewer")+"/pc_tmp.ply").c_str());
		std::ofstream f((ros::package::getPath("ar_viewer")+"/pc_tmp.ply").c_str());
		int numpts = 0;
		for(unsigned int i=0;i<keyframes.size();i++)
		{
		...
		std::ofstream f2((ros::package::getPath("ar_viewer")+"/pc.ply").c_str());
		f2 << std::string("ply\n");
		...
		std::ifstream f3((ros::package::getPath("ar_viewer")+"/pc_tmp.ply").c_str());
		while(!f3.eof()) f2.put(f3.get());

		f2.close();
		f3.close();

		system(("rm "+ros::package::getPath("ar_viewer")+"/pc_tmp.ply").c_str());
		flushPointcloud = false;
...
void KeyFrameGraphDisplay::addMsg(ar_viewer::keyframeMsgConstPtr msg)
{
	dataMutex.lock();
...
void KeyFrameGraphDisplay::addGraphMsg(ar_viewer::keyframeGraphMsgConstPtr msg)
{
	dataMutex.lock();
